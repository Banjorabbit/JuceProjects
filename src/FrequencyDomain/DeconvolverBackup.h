...
class Deconvolver : public BaseFrequencyDomain<Deconvolver, I::Deconvolver>
{
	...
	struct Coefficients
	{
		...
		int N = 20; // Filter length for one channel ( total filter length is N + NLow)
	...
	struct Data
	{
		Eigen::LLT<Eigen::MatrixXcf> Solver;
	...
	void ProcessOn(Input input, Output yFreq)
	{
		...
		for (auto ibin = 0; ibin < C.NBands; ibin++)
		{
			...
			if (input.speechActivity)
			{
				...
				for (auto i = 0; i < D.FilterLength; i++) // only calculate lower triangular part of covariance matrix
				{
					D.RCov[ibin].block(i, i, D.FilterLength - i, 1) += invTheta * BufferInput.tail(D.FilterLength - i) * std::conj(BufferInput(i));
				}
			...
			if (updateFilterBin == 0)
			{
				...
				D.Filter[ibin] = D.Solver.compute(cov).solve(D.RCross[ibin]);
			}
			updateFilterBin++;
