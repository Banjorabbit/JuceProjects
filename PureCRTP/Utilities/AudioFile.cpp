...
#pragma warning( disable : 4244 4245 4389 4457 4458)
...
template <class T>
AudioFormatType AudioFile<T>::getFormat() const
{
	return static_cast<AudioFormatType>(AudioFormat);
}
...
template <class T>
void AudioFile<T>::setAudioFormat(AudioFormatType newAudioFormat)
{
...
bool AudioFile<T>::load (const std::string& filePath)
{
	...
	// MKM edit of file  read
	file.seekg(0, std::ios_base::end);
	std::size_t size = file.tellg();
	file.seekg(0, std::ios_base::beg);
	std::vector<uint8_t> fileData(size/sizeof(uint8_t));
	file.read(reinterpret_cast<char*>(&fileData[0]), size);
	file.close();
    // MKM edit of file  read
...
bool AudioFile<T>::decodeWaveFile (std::vector<uint8_t>& fileData)
{
    ...
    AudioFormat = static_cast<AudioFormatType>(twoBytesToInt (fileData, f + 8));
    ...
    // check that the audio format is PCM or Floating Point (Format 3)
    if (!(AudioFormat == AudioFormatType::PCM || AudioFormat == AudioFormatType::Floating))
    {
    ...
    // check bit depth is either 8, 16 or 24 bit PCM or 32 bit Floating Point
    if (bitDepth != 8 && bitDepth != 16 && bitDepth != 24 && (bitDepth != 32 && AudioFormat == AudioFormatType::Floating))
    {
        std::cout << "ERROR: this file has a bit depth that is not 8, 16 or 24 bits PCM or 32 bit Floating Point" << std::endl;
        return false;
    }
    ...
    for (int i = 0; i < numSamples; i++)
    {
        for (int channel = 0; channel < numChannels; channel++)
        {
            ...
            if (bitDepth == 8)
            {
	            auto sampleAsInt = (int32_t) fileData[sampleIndex];
			...
			// Floating Point
			else if (bitDepth == 32 && AudioFormat == AudioFormatType::Floating)
			{
				// Read the bytes as floats
				const auto pFileData = reinterpret_cast<float*>(&fileData[sampleIndex]);
				samples[channel].push_back(*pFileData);
			}
...
bool AudioFile<T>::decodeAiffFile (std::vector<uint8_t>& fileData)
{
    ...
    if ((soundDataChunkSize - 8) != totalNumAudioSampleBytes || totalNumAudioSampleBytes > static_cast<int>(fileData.size() - samplesStartIndex))
    {
...
bool AudioFile<T>::SaveToWaveFile (const std::string& filePath)
{
    ...
    const auto dataChunkSize = getNumSamplesPerChannel() * (getNumChannels() * bitDepth / 8);
	...
	if(AudioFormat == AudioFormatType::Floating)
	{
		// Extra bytes for floating format chunk
		fileSizeInBytes += 2; 
	}
	...
	switch (AudioFormat)
	{
		...
		break;
		case AudioFormatType::Floating:
			addInt32ToFileData(fileData, 18); // format chunk size for Floating (16 + 4)
		break;
	}
    //addInt32ToFileData (fileData, 16); // format chunk size (16 for PCM)
    addInt16ToFileData (fileData, static_cast<int16_t>(AudioFormat)); // audio format (1 = PCM, 3 = Float)
    ...
    auto numBytesPerSecond = (int32_t) ((getNumChannels() * sampleRate * bitDepth) / 8);
	...
	// If Non-PCM two extra fields are needed
	if(AudioFormat == AudioFormatType::Floating)
	{
		// Extension size - zero for IEEE floating
    ...
    for (int i = 0; i < getNumSamplesPerChannel(); i++)
    {
        for (int channel = 0; channel < getNumChannels(); channel++)
        {
            if (bitDepth == 8)
            {
	            auto sampleAsInt = static_cast<int32_t>((samples[channel][i] * static_cast<T>(128.)) + 128.);
	            auto byte = (uint8_t)sampleAsInt;
                fileData.push_back (byte);
            }
            else if (bitDepth == 16)
            {
	            auto sampleAsInt = (int16_t) (samples[channel][i] * (T)32768.);
			...
			{
				const auto floatingPointValue = reinterpret_cast<uint8_t*>(&samples[channel][i]);

				for (auto byte = 0; byte < 4; byte++)
				{
					fileData.push_back(floatingPointValue[byte]);
				}
			}
            else
            {
...
bool AudioFile<T>::saveToAiffFile (const std::string& filePath)
{
    ...
    for (int i = 0; i < getNumSamplesPerChannel(); i++)
    {
        for (int channel = 0; channel < getNumChannels(); channel++)
        {
            if (bitDepth == 8)
            {
	            auto sampleAsInt = (int32_t)(samples[channel][i] * (T)128.);
	            auto byte = (uint8_t)sampleAsInt;
                fileData.push_back (byte);
            }
            else if (bitDepth == 16)
...
//=============================================================
template <class T>
bool AudioFile<T>::writeDataToFile (std::vector<uint8_t>& fileData, const std::string& filePath)
{
...
//=============================================================
template <class T>
int AudioFile<T>::getIndexOfString (std::vector<uint8_t>& source, const std::string& stringToSearchFor)
{
    int index = -1;
    const int stringLength = (int)stringToSearchFor.length();
...
#pragma warning( default : 4244 4245 4389 4457 4458)